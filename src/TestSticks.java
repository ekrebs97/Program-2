...
public class TestSticks {
	...
	// testing class
	private static final int NUM_ACTIONS = Config.MAX_ACTION - Config.MIN_ACTION + 1;
	 ...
	 * 
	 * 1. If the number of sticks remaining is 0, then the computer should take
	 * 0 sticks. If the response is not 0, then an error message is printed.
	 * 
	 * 2. If the number of sticks remaining is a negative number, then the
	 * computer should take 0 sticks. If the response is not 0, then an error
	 * message is printed.
	 * 
	 * 3. If the number of sticks remaining is 2, then the computer should
	 * choose the minimum number of sticks available. If the response is not the
	 * minimum (1), then an error message is printed.
	 * 
	 * 4. If there are 10 sticks remaining, then the computer should choose
	 * between the minimum or maximum number of sticks available defined in the
	 * method. If the response is not between that range, then an error message
	 * is printed.
	 * 
	 * 5. A random number generator is set to a certain seed to return 3
	 * specific numbers. When the number of sticks remaining is 10, then the
	 * array will pass 3 specified numbers in each array index that corresponds
	 * to the seed value. If the response does not have those numbers in the
	 * specific index, then an error message is printed.
	 */
	private static void testBasicChooseAction() {
		...
		if (response != 0) {
			error = true;
			System.out.println("testBasicChooseAction 1: for 0 sticks, " + "response should be 0.");
		}
		...
		if (response != 0) {
			error = true;
			System.out.println("testBasicChooseAction 2: for negative sticks, " + "response should be 0.");
		}
		...
		if (response != 1) {
			error = true;
			System.out.println("testBasicChooseAction 3: for 2 sticks, " + "response should be 1.");
		}
		...
		if (response < Config.MIN_ACTION || response > Config.MAX_ACTION) {
			error = true;
			System.out.println("testBasicChooseAction 4: for 10 sticks," + " response should be between "
					+ Config.MIN_ACTION + " or " + Config.MAX_ACTION);
		}
		...
		if (responses[0] != 329 || responses[1] != 339 || responses[2] != 332) {
			...
			System.out.println("testBasicChooseAction 5: for seed 123 "
					+ "responses were expected to be [329, 339, 332] " + " but found " + Arrays.toString(responses));
	 ...
	 * 
	 * 1. If the variable action does not equal zero, then an error is printed
	 * saying that the response should be 0.
	 * 
	 * 2. If the computer returns a negative number of sticks, an error is
	 * printed saying that that the response should be 0.
	 * 
	 * 3. If the number of sticks taken is out of bounds, an error is printed
	 * saying that it's an invalid action.
	 * 
	 * 4. If the action is not set to the maximum number of sticks that can be
	 * taken, an error is printed saying that the max sticks was expected
	 * 
	 * 5. A seed is set to have the computer return a certain pattern of
	 * numbers. If this is not the case, an error is printed
	 * 
	 */
	private static void testAiChooseAction() {
		...
		if (action != 0) {
			error = true;
			System.out.println("testAiChooseAction 1: for 0 sticks or null " + "actionRanking, response should be 0.");
		}
		...
		if (action != 0) {
			error = true;
			System.out.println("testAiChooseAction 2: for negative sticks," + " response should be 0.");
		}
		...
		if (action < Config.MIN_ACTION || action > Config.MAX_ACTION) {
			error = true;
			System.out.println("testAiChooseAction 3: invalid action " + action);
		}
		...
		if (action != Config.MAX_ACTION) {
			error = true;
			System.out.println("testAiChooseAction 4: expected " + Config.MAX_ACTION + " rather than " + action);
		}
		...
		if (responses[0] != 1037 || responses[1] != 5819 || responses[2] != 3144) {
			error = true;
			System.out.println("testAiChooseAction 5: for seed 123 "
					+ "responses were expected to be [1037, 5819, 3144] " + " but found " + Arrays.toString(responses));
	...
	/**
	 * This method runs tests on the initializeActionRanking method. 1. Once the
	 * initializeActionRanking is initialized, it should simply return
	 * 
	 * 2. If the number in each actionRanking index does not equal 1, then an
	 * error is printed, saying that it has a non-one value
	 * 
	 * 3. The actionRanking array is being initialized to just zero values. If
	 * the number in each actionRanking index does not equal 1, then print an
	 * error saying that it has a non-one value
	 * 
	 */
	private static void testInitializeActionRanking() {
		...
		for (int i = 0; i < actionRanking.length; i++) {
			if (actionRanking[i] != 1) {
				error = true;
				System.out.println("testInitializeActionRanking 2: index " + i + " has a non-one value: "
		...
		for (int i = 0; i < actionRanking.length; i++) {
			if (actionRanking[i] != 1) {
				error = true;
				System.out.println("testInitializeActionRanking 3: index " + i + " has a non-one value: "
						+ Arrays.toString(actionRanking));
			}
		}
	 ...
	 * 
	 * 1. A certain output is expected, changing the integers to strings and
	 * including a tab and newline character. If the output is not matched, an
	 * unexpected output error is printed
	 * 
	 * 2. If the returned string does not end with a newline character, an error
	 * is printed stating that the output did not end with a newline character
	 */
	private static void testActionRankingToString() {
		...
		if (!out.equals(expected)) {
			System.out.println("testActionRankingToString 1: Unexpected " + "output: " + out);
			error = true;
		}

		// 2.
		if (!out.endsWith("\n")) {
			System.out.println("testActionRankingToString 2: output didn't " + "end with \\n");
			error = true;
		}
	 ...
	 * 
	 *         Allows us to use the action variable in our Sticks file
	 */
	...
	/**
	 * This method tests the updateActionRankingOnLoss method. 1. The
	 * actionRanking array at actionIndex must be 1 because the array must be
	 * decremented by one. If not, an error message is printed.
	 */
	private static void testUpdateActionRankingOnLoss() {
		...
		if (actionRanking[actionIndex] != 1) {
			System.out.println("testUpdateActionRankingOnLoss 1: Unexpected " + "actionRanking contents: "
					+ Arrays.toString(actionRanking));
			error = true;
		}
	...
	/**
	 * This method tests the updateActionRankingOnWin method. 1. The
	 * actionRanking array at an index must be 1 in order for this method to
	 * increment the actionRanking array by 1.
	 */
	private static void testUpdateActionRankingOnWin() {
		...
		if (actionRanking[index] != 1) {
			...
			System.out.println("testUpdateActionRankingOnWin 1: Unexpected " + "actionRanking contents: "
					+ Arrays.toString(actionRanking));
			error = true;
		}
	 ...
	 * 
	 * 1. If the strategyTable's length does not equal 30 (10 rows, 3 columns)
	 * then an error message is printed.
	 * 
	 * 2. If the strategyTable's length at index 0 does not equal NUM_ACTIONS,
	 * and error message is printed.
	 * 
	 * 3. If the number at strategyTable's first index does not equal 1, an
	...
	private static void testCreateAndInitializeStrategyTable() {
		...
		if (strategyTable.length != 30) {
			System.out.println("testCreateAndInitializeStrategyTable 1: "
					+ " strategy table expected length was 30, rather than : " + strategyTable.length);
			error = true;
		}

		// 2.
		if (strategyTable[0].length != NUM_ACTIONS) {
			System.out.println("testCreateAndInitializeStrategyTable 2: " + " strategy table expected width was "
					+ NUM_ACTIONS + " , rather than : " + strategyTable[0].length);
			error = true;
		}
		...
		if (strategyTable[0][0] != 1) {
			System.out.println(
					"testCreateAndInitializeStrategyTable 3: " + " strategy table should be initialized to all 1's");
			error = true;
		}
	 ...
	 * 
	 * 1. This method is testing that the strategyTableToString method is
	 * returning the correct string that should be printed. If not, an error
	 * message is printed.
	 */
	private static void testStrategyTableToString() {
		...
		// 1.
		int[][] strategyTable = new int[][] { { 1, 10, 11 }, { 4, 6, 2 }, { 1, 1, 1 }, { 6, 3, 1 }, { 4, 4, 4 },
				{ 1, 4, 1 }, { 1, 5, 1 }, { 3, 1, 1 }, { 1, 1, 7 }, { 1, 2, 1 } };
		...
		if (!str.equals(expected)) {
			System.out.println(
					"testStrategyTableToString 1: unexpected output " + " expected: " + expected + "\nwas:" + str);
			error = true;
		}
	 ...
	 * 
	 * 1. Specific actions are initialized to certain indexes in the
	 * strategyTable array, and if they don't equal to 1 (actionRanking is not
	 * updated, not incremented by 1), an error message is printed
	 */
	private static void testUpdateStrategyTableOnWin() {
		...
		// have been updated.
		if (strategyTable[2][action1Index] != 1 || strategyTable[5][action2Index] != 1) {
			System.out.println("testUpdateStrategyTableOnWin 1: unexpected " + "value in strategy table:"
	 ...
	 * 1. Specific actions are initialized to certain indexes in the
	 * strategyTable array, and if they don't equal to 1 (actionRanking is not
	 * updated, not decremented by 1), an error message is printed
	 */
	private static void testUpdateStrategyTableOnLoss() {
		...
		// have been updated.
		if (strategyTable[2][action1Index] != 1 || strategyTable[5][action2Index] != 1) {
			System.out.println("testUpdateStrategyTableOnLoss 1: unexpected " + "value in strategy table:"
					+ Sticks.strategyTableToString(strategyTable));
			error = true;
		}
	 ...
	 * 
	 * 1. Check that strategyTable must be initialized to 1. Check to see that
	 * the winner's return value is 1 or 2, and that the right number is being
	 * returned. Check to see which player picked up the last stick. Check that
	 * all the values in actionHistory are valid. Also check to see that the
	 * number of sticks chosen is not out of bounds
	 */
	private static void testPlayAiVsAi() {
		...
		int winner = Sticks.playAiVsAi(startSticks, p1StrategyTable, p1ActionHistory, p2StrategyTable, p2ActionHistory);

		// see if return value is valid
		if (winner < 1 || winner > 2) {
			System.out.println("testPlayAiVsAi 1: winner should be 1 or 2, " + "not: " + winner);
			error = true;
		}
		...
		if (lastStick >= 1 && lastStick <= 2 && winner == lastStick) {
			System.out.println("testPlayAiVsAi 2: winner should be returned " + "not loser.");
			error = true;
		}
		...
		for (int i = 0; i < p1ActionHistory.length; i++) {
			...
			if (action != 0) {
				if (action < Config.MIN_ACTION || action > Config.MAX_ACTION) {
					System.out
							.println("testPlayAiVsAi 3: invalid action: " + action + " at p1ActionHistory[" + i + "]");
					error = true;
				}
			}
		}
		...
		for (int i = 0; i < p2ActionHistory.length; i++) {
			...
			if (action != 0) {
				if (action < Config.MIN_ACTION || action > Config.MAX_ACTION) {
					System.out
							.println("testPlayAiVsAi 4: invalid action: " + action + " at p2ActionHistory[" + i + "]");
					error = true;
				}
			}
		}
	 ...
	 * 
	 * 1. If startSticks is less than the minimum allowed value, or strategyTable is null, an error message is printed.
	 * 
	 * 2. If the strategyTable's length is not equal to startSticks, then an error message is returned.
	 * 
	 * 
	 */
	private static void testTrainAi() {
		...
		if (startSticks < Config.MIN_STICKS && strategyTable != null) {
			System.out.println("testTrainAi 1: expected null strategy table for " + " startSticks:" + startSticks
		...
		if (strategyTable.length != startSticks) {
			System.out.println("testTrainAi 2: unexpected strategy table length: " + strategyTable.length
					+ " expected: " + startSticks);
			error = true;
		}

		if (strategyTable[0].length != NUM_ACTIONS) {
			System.out.println("testTrainAi 3: unexpected strategy table width: " + strategyTable[0].length
